// ignore_for_file: no_leading_underscores_for_local_identifiers, unrelated_type_equality_checks, deprecated_member_use
...
//base url
String url = 'https://tochegandodelivery.site'; //add '/' at the end of the url as 'https://url.com/'
String mapkey =
    (platform == TargetPlatform.android) ? 'android map key' : 'AIzaSyDp58F_Sdf-CrcwUb8ZizIV7zCVEjIB1FI';
...
darktheme() async {
  if (isDarkTheme == true) {
    ...
    buttonColor = buttonColorLightMood;
      ...
      if (choosenLanguage.isNotEmpty) {
        if (pref.containsKey('Bearer')) {
          ...
          if (tokens != null) {
            ...
            if (responce == true) {
              if (userDetails['role'] != 'owner' && Platform.isAndroid) {
...
geoCodingForLatLng(id, sessionToken) async {
  try {
    ...
    val = await http.get(
        Uri.parse(
            'https://places.googleapis.com/v1/places/$id?fields=id,displayName,location&key=$mapkey'),
        headers: {
    ...
    if (val.statusCode == 200) {
      ...
      // var result = LatLng(value['location']['latitude'], value['location']['longitude']);
      var result = value['location'];
      return result;
    }
  } catch (e) {
    debugPrint(e.toString());
...
getPolylines(animate) async {
  ...
  polyList.clear();
  // polyline.clear();
  if (isDestinationChanged == true && mapType != 'google') {
    fmpoly.clear();
  }
  String pickLat = '';
  String pickLng = '';
...
streamRide() {
  ...
  rideStreamChanges = FirebaseDatabase.instance
    ...
    if (event.snapshot.key.toString() == 'cancelled_by_user') {
      ...
      getUserDetails();
    } else if (event.snapshot.key.toString() == 'destination_change') {
      polyString = '';
      polyline.clear();
      polyList.clear();
  ...
  rideStreamStart = FirebaseDatabase.instance
    ...
    if (event.snapshot.key.toString() == 'cancelled_by_user') {
      ...
      getCurrentMessages();
    } else if (event.snapshot.key.toString() == 'modified_by_user') {
      getUserDetails();
    } else if (event.snapshot.key.toString() == 'is_paid' ||
        event.snapshot.key.toString() == 'modified_by_user') {
      getUserDetails();
...
streamAdminchat() async {
  chatStream = FirebaseDatabase.instance
      ...
      .onValue
      .listen((event) async {
    var value = event.snapshot.value != null
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ? geolocs.AndroidSettings(
        ...
        foregroundNotificationConfig:
          ...
          notificationText:
              "To Chegando Delivery Motoboy to receive your location in background",
...
additionalCharge(reason, amount) async {
  dynamic result;
  try {
    var response =
        await http.post(Uri.parse('${url}api/v1/request/additional-charge'),
            headers: {
              'Authorization': 'Bearer ${bearerToken[0].token}',
              'Content-Type': 'application/json'
            },
